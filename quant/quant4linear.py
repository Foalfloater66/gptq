...
class Quant4Linear(nn.Module):
    ...
    # Revert signature: pack takes original linear layer, scale, zero_point
    def pack(self, linear, scales, zeros_float):
        ...
        Args:
            ...
            scales (torch.Tensor): The quantization scales (shape: [outfeatures, 1])
                                   obtained from the quantizer used in GPTQ.
        ...
        target_device = scales.device # Use device of scales/zeros
        ...
        # --- Re-quantize original weights using the exact GPTQ parameters ---
        # Q(x) = round(x / scale + zero_point)
        weight_fp = linear.weight.data.to(target_device) # Ensure original weights are on target device
